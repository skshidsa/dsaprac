...
{
node * root = new node;
strcpy(root->word,word); strcpy(root->meaning,meaning);
root->lptr=NULL;
root->rptr=NULL;
return root;
}
...
if(root->lptr==NULL)
{
cout<<"Word Not Found!!!!"<<endl; return NULL;
}
else
{
...
node * min;
min=root->rptr; while(min->lptr!=NULL)
{
min=min->lptr;
}
temp=min; strcpy(root->word,temp->word); strcpy(root->meaning,temp->meaning); root->rptr=del(temp->word,root->rptr); return root;
...
cout<<"Word :"<<root->word<<endl; cout<<"Meaning :"<<root->meaning<<endl;
}
else if(strcmp(word,root->word)<0)
{
if(root->lptr==NULL)
...
{
char meaning[20];
cout<<"Enter Meaning of the word "<<root->word; cin>>root->meaning;
return;
}
else if(strcmp(word,root->word)<0)
...
if(root!=NULL)
{
ascorder(root->lptr); cout<<root->word<<"=>"<<root->meaning<<endl; ascorder(root->rptr);
}
}
...
if(root!=NULL)
{
descorder(root->rptr); cout<<root->word<<"=>"<<root->meaning<<endl; descorder(root->lptr);
}
}
};
int main()
...
node * root = NULL;
Dictionary dict;
char word[20],meaning[20],searchword[20],updateword[20],deleteword[20]; while(1)
{
...
cout<<"Enter Word to be Updated"; cin>>updateword; dict.update(updateword,root); break;
case 5: dict.ascorder(root); break;
case 6: dict.descorder(root); break;
case 7:
break;
default : 
cout<<"Invalid Choice"; 
